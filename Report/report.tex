...
The Coq proof assistant was developed in 1984 [...] %\cite{Coquand & Huet}
and its logic, originally based on the Calculus of Constructions, was extend to the
Calculus of Inductive Constructions in 19xx [...]. %YEAR and \cite{Christe Paulin}
It has been under constant development ever since it was first published and many
bugs have since been found and fixed, making Coq one of the most reliable and trusted
proof environments.
...
that it all still works when adding probabilistic functions. In order to do this, we
encapsulate all of our computations in the monadic structure presented in Section
\ref{sec:monad-interp}. This means that the result of a computation is no longer a
definite values anymore, but rather a monadic value representing a probability
distribution over results of the internal computation.

This is where the argument from Section \ref{sec:prob-rec} comes into play. This
tells us that we can add probability to our languages without changing the
interpretation of recursive definitions.

We also note that the probabilistic computations for both \rml\ and \texttt{pwhile}
