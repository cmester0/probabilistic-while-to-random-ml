...
The first approach to proving something about randomised algorithms in Coq that we
will examine here is due toe Philippe Audebaud and Christine Paulin-Mohring %\cite
and features the functional language \rml. 
\\ \\
We have two representations of Rml, continuations and distributions. Both build on a
monad, for ease of use.
\\ \\
...
Most of the above constructs are well known and need no special explanation. What is
worth noting, however, is \texttt{prp (pred mem)}: \texttt{pred} is the
\todo{Is there a word like 'functional' for types?}type transformer $\forall \tau:
\tau \to \mathbb{B}$, while \texttt{mem} is a type that is defined by the
\texttt{xhl} development in order to represent memories for \texttt{pwhile} programs.  

\todo[inline]{What do the predicates do? Types for constants. }
\todo[inline, color=green!40]{The xhl version of \texttt{pwhile} is a shallow embedding in
  Coq. This means that constants are nothing other than Gallina values; note that since
  Gallina is a functional language, functions can be viewed as values as well and
  hence it makes sense to have function application as an expression even though we
  have no ``real'' way of defining functions in \texttt{pwhile.} }
\todo[inline, color=green!40]{\texttt{pred} is an ssreflect Type
  transformer. Something of type \texttt{pred T} is a function that takes as input an
  element of type T and returns a boolean value. So a \texttt{pred mem} is simply a
  function that takes a memory and returns a boolean value depending on the state of
  said memory. }
\todo[inline] {Is \texttt{prp} needed for expressiveness? }
