...
\begin{align}
  ...
  x~\mapsto~ & x\\
  c~\mapsto~ & c\\
  \texttt{let }x := e_1\texttt{ in }e_2 ~\mapsto~
             & (\lambda x.e_2)~e_1 \\
  \texttt{if $b$ then $e_1$ else }e_2~\mapsto~
             & b~e_1~e_2 \label{eq:lambda-if}\\
  f~e_1\dots e_n ~\mapsto~
             & f~e_1\dots e_n\\
  \texttt{let rec }f~x:= e_1 \texttt{ in } e_2 ~\mapsto~
             & (\lambda f.e_2)~(\texttt{Y}~(\lambda f.\lambda x. (e_1~x)))
\end{align}
