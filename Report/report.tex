...
is done by the function \coqe{replace_all_variables_type}. \todo{refer to appendix
  with this? }

\todo[inline]{Why is \srml\ parameterised by a type? }

\subsection{Typing \rmlx}
\todo[inline]{We need something about what it means for \texttt{Rml} to be well-typed,
  something about the validity of both translations as well as the translation
  itself.

  Maybe the stuff about the translation would fit better in the previous section, but
  we definitely need one for the type-checker and some of our beautiful
  lemmas/theorems. }

Before we want to translate a program in \rmlx\ to one in \srml, we want to be sure
that the program we are trying to interpret does not ``go wrong''. Having learned
from Robin Milner %\cite{} "A Theory of Type Polymorphism in Programming", 1978
we know that in order to ensure this we must implement some typing rules and a way to
check that a program is well-typed.

The rules we implemented are mostly straightforward and can be found in Appendix
\ref{appendix:rmltypes}. 


\subsection{Something something interpretation}
\todo[inline]{No, this is not going to be the final title of this subsection, but I
  needed a placeholder here, in order to have the structure of the section in place.}
