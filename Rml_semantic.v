...
Fixpoint ssem {R} {T} (x : Rml) `{x_valid : rml_valid_type T x nil} : distr R (Choice T) :=
  let y := @replace_all_variables x (@valid_is_well x T nil x_valid) in
  let y_simple := @replace_var_for_let_simple x T x_valid in
  let y_valid := @replace_var_for_let_valid x T x_valid in
 ...
 *)
(* Compute @ssem R nat (Const nat 4) (@valid_const nat nat 4 (@erefl Type nat) nil). *)
