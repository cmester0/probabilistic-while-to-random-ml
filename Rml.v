...
Proof.
  ...
  - (* apply (replace_all_variables_aux_type _ (Let_stm (_,_) (*f*) (dlim _))).  *)
    ...
    assert (x1_valid : rml_valid_type T x1 ((n,T) :: (n0,T0) :: [seq i.1 | i <- env]))
      by (inversion x_valid; subst; assumption).

    (* Added *)
    assert (valid_env_ext : forall f : T, forall x : T0, valid_env [:: (n, T, Const T f), (n0, T0, Const T0 x) & env]) by (repeat try constructor ; try easy).

    pose (fun f : T =>
            (fun (x : T0) =>
               replace_all_variables_aux_type T x1 [:: (n,T,Const T f) , (n0,T0,Const T0 x) & env] (valid_env_ext f x) x1_valid : @sRml T)).
